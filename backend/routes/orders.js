...
router.post("/", validateCreateOrder, async (req, res) => {
  try {
    const { collegeId, collegeName, items, student, guardian, delivery, coupon, paymentMethod } = req.body;
    ...
    const order = await createOrder({ collegeId, collegeName, items, student, guardian, delivery, coupon, paymentMethod });
